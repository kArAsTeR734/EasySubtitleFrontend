...
export const ResultStep: FC<ResultStepProps> = ({ fileId, processingResult, selectedFile }) => {
  //const [script, setScript] = useState<GetTranscriptionResult | null>(null);
  const { error: transcriptionError, fetching: fetchTranscription } = useFetching<GetTranscriptionResult, [string]>(
    getTranscription,
    {
      onSuccess: (data) => {
  ...
  const result = selectedFile?.text ? { scripts: selectedFile.text } : processingResult;
  useEffect(() => {
    if (fileId) {
