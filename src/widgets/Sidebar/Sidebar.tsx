...
const Sidebar = () => {
  ...
  const { fetching: uploadFileFetching } = useFetching<SideBarTranscriptionList, [number, number]>(
    ...
    >
      <div className="sidebar__header">
        ...
        <div className="sidebar__header-right">
          ...
          >
            <span>{isOpen ? <LeftOutlined className="arrow-icon" /> : <RightOutlined className="arrow-icon" />}</span>
          </Button>
        </div>
      </div>
      {isAuthorizated ? <MenuList data={files} /> : <p>Войдите чтобы читать файлы</p>}
      <div
